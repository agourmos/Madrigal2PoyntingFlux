    ...
    if sum(vals_switch[0:6]) == 6: # Check date and time
        ...
        if sum(vals_switch[6:8]) == 2 or sum(vals_switch[8:10]) == 2:
            ...
            if settings['plotMF'] == 1 and not sum(vals_switch[11:13]) == 2:
                print('PlotError: Magnetic field component(s) not given for plotting.')
                sys.exit()
            ...
            # Check to see if velocity values were given for plotting
            if settings['plotVF'] == 1 and not sum(vals_switch[13:15]) == 2:
                print('PlotError: Velocity field component(s) not given for plotting.')
                sys.exit()
                
            # If values are turned on, turn them off. Helps with indexing
            elif settings['plotVF'] == 0 and sum(vals_switch[13:15]) == 2:
                vals_switch[13:15] = 0
                
            # Check to see if electric values were given for plotting
            if settings['plotEF'] == 1 and not sum(vals_switch[15:17]) == 2:
                print('PlotError: Electric field component(s) not given for plotting.')
                sys.exit()
                
            # If values are turned on, turn them off. Helps with indexing
            elif settings['plotEF'] == 0 and sum(vals_switch[15:17]) == 2:
                vals_switch[15:17] = 0
                        
            # Check to see if poynting flux is given
            if vals_switch[10] == 1:
                # All good, reduce and output positional values
