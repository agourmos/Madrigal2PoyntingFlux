...
from functions.spherical_geometry2 import (angle_difference,
                                            angle_midpoint,
...
def draw_dialplot(ax,minlat,padding=3,fslt=10,fslat=12,southern_hemi=False,
    ...
    if latlabels:
        ...
        if minlat < 60:
            ax.text(36,-5,sh+r'$50^o$',fontsize=fslat,color=tcolor);

        if minlat < 50:
            ax.text(46,-5,sh+r'$40^o$',fontsize=fslat,color=tcolor);
...
def hairplot(fig,ax,lat,lt,C,hemisphere,boundsDict,settings,max_size=10,max_val=None,vmin=None,vmax=None,ref_units=None,dialplot=True,horizontal=False,min_displayed=0.,**kwargs):
    """
    ...
    if dialplot:
        if lat[0] < 0:
            draw_dialplot(ax,40,southern_hemi=True)
        else:
            draw_dialplot(ax,40)
    ...
    X,Y = latlt2cart(lat,lt,hemisphere) # Flight path positions in cartesian
    if not horizontal:
        X1 = zeros_like(X)
    ...
    Q = ax.quiver(X[above_min],Y[above_min],X1[above_min],Y1[above_min],color = cm(norm(C[above_min])),
        angles='xy',units='xy',width=.4,scale_units='xy',scale=0.5,headwidth=0,headlength=0,norm=norm,**kwargs)

    #Q appears to not actually create a mappable??
    mappable = matplotlib.cm.ScalarMappable(norm=Q.norm,cmap=cm)
    ...
    # These are added plot mechanisms by Adam Gourmos
    # Change color of flight path
    plt.sca(ax)
    cbar = plt.colorbar(mappable)
    cbar.ax.set_ylabel(r'$\frac{W}{m^2}$',rotation = 0, labelpad = 20,fontsize=14)
    
    # Determine whether or not to add fixed bars or keep variable
    if boundsDict['boundsType'] == 1 or boundsDict['boundsType'] == 2:
        cbar.mappable.set_clim(vmin=boundsDict['bounds'][0],vmax=boundsDict['bounds'][1])
    
    # Add colorbar to plot
    
    # Plot solid black flight path
    #plt.sca(ax[0])
    plt.scatter(X,Y,s= 2, color='k')
    
    # Build quivers to show direction of satellite path
    u = np.diff(X)
    v = np.diff(Y)
    pos_x = X[:-1] + u/2
    pos_y = Y[:-1] + v/2
    ...
    # Add direction of flight path
    ax.quiver(pos_x[0::200], pos_y[0::200], u[0::200]/norm[0::200],v[0::200]/norm[0::200],
              color = 'Black', angles="xy", zorder=5, pivot="mid")

    return mappable

def hairplot2(fig,ax,lat,lt,MFNS,MFEW,hemisphere,settings,max_size=10,max_val=None,vmin=None,vmax=None,ref_units=None,dialplot=True,horizontal=False,min_displayed=0.,**kwargs):
    """
    Makes top-down polar plots with vertical lines to indicate intensity and color along spacecraft track.
    Can handle either an array of colors or a variable of colors.
    """
    # Define frequency of vectors to be plotted
    N = 20
                
    # Reduce the data that is being input into plotting function
    lat2 = lat[1:-1:N]
    lt2 = lt[1:-1:N]
    MFNS2 = MFNS[1:-1:N]
    MFEW2 = MFEW[1:-1:N]
    ...
    # Find magnitude of magnetic field vector
    C = []
    [C.append(np.sqrt(MFNS2[idx]**2 + MFEW2[idx]**2)) for idx,val in enumerate(MFNS2)]
    C = np.array(C)
    ...
    X,Y = latlt2cart(lat,lt,hemisphere) # Flight path positions in cartesian
    X2,Y2 = latlt2cart(lat2,lt2,hemisphere) # Flight path positions in cartesian
    ...
    Q = ax.quiver(X2,Y2,MFEW2*6e7,MFNS2*6e7,color = cm(norm(C[above_min])),
        angles='xy',units='xy',width=0.75,scale_units='xy',scale=1,
        headwidth=0,headlength=0,norm=norm,**kwargs)
    ...
    plt.sca(ax)
    cbar = plt.colorbar(mappable)
    cbar.ax.set_ylabel(r'$nT$',rotation = 0, labelpad = 20,fontsize=14)
    ...
    # Plot solid black flight path
    #plt.sca(ax[0])
    plt.scatter(X,Y,s= 0.05, color='k')
...
def hairplot3(fig,ax,lat,lt,MFNS,MFEW,hemisphere,settings,max_size=10,max_val=None,vmin=None,vmax=None,ref_units=None,dialplot=True,horizontal=False,min_displayed=0.,**kwargs):
    """
    Makes top-down polar plots with vertical lines to indicate intensity and color along spacecraft track.
    Can handle either an array of colors or a variable of colors.
    """
    ...
    Q = ax.quiver(X2,Y2,MFEW2*.01,MFNS2*.01,color = cm(norm(C[above_min])),
        angles='xy',units='xy',width=0.75,scale_units='xy',scale=0.5,
        headwidth=0,headlength=0,norm=norm,**kwargs)
    ...
    plt.sca(ax)
    cbar = plt.colorbar(mappable)
    cbar.ax.set_ylabel(r'$\frac{m}{s}$',rotation = 0, labelpad = 20,fontsize=14)
...
def hairplot4(fig,ax,lat,lt,MFNS,MFEW,hemisphere,settings,max_size=10,max_val=None,vmin=None,vmax=None,ref_units=None,dialplot=True,horizontal=False,min_displayed=0.,**kwargs):
    """
    Makes top-down polar plots with vertical lines to indicate intensity and color along spacecraft track.
    Can handle either an array of colors or a variable of colors.
    """
    ...
    Q = ax.quiver(X2,Y2,MFEW2*100,MFNS2*100,color = cm(norm(C[above_min])),
        angles='xy',units='xy',width=0.75,scale_units='xy',scale=0.25,
        headwidth=0,headlength=0,norm=norm,**kwargs)
    ...
    plt.sca(ax)
    cbar = plt.colorbar(mappable)
    cbar.ax.set_ylabel(r'$\frac{V}{m}$',rotation = 0, labelpad = 20,fontsize=14)
...
def hairplotT(fig,ax,lat,lt,C,hemisphere,boundsDict,settings,switch,max_size=10,max_val=None,vmin=None,vmax=None,ref_units=None,dialplot=True,horizontal=False,min_displayed=0.,**kwargs):
    """
    Makes top-down polar plots with vertical lines to indicate intensity and color along spacecraft track.
    Can handle either an array of colors or a variable of colors.
    """
    ...
    # Check to see if values are empty given reduction
    if not lat.tolist():
        return ax, switch
    ...
    # Define plot we are using
    plt.sca(ax)
    
    # Define opacity levles
    ops = (C-boundsDict['bounds'][1])/(boundsDict['bounds'][0]-boundsDict['bounds'][1])*1
    
    # Change all NaN values to 0 for opacity as well as out of bound numbers
    ops[isnan(ops)] = 0
    ops[ops>1] = 1
    ops[ops<0] = 0
    
    # Check to see if plot is empty
    if not any(C):
        return ax, switch
    else:
        # Plot based on whether or not the first plot has been created
        if switch == 1:
            if boundsDict['boundsType'] == 1 or boundsDict['boundsType'] == 2:
                plt.scatter(X,Y,s= 7, c = C,cmap=matplotlib.cm.cool,alpha=ops,marker = "s")
            else:
                plt.scatter(X,Y,s= 25, c = C,cmap=matplotlib.cm.cool,alpha=1,marker = "|")
            ...
            if boundsDict['boundsType'] == 1 or boundsDict['boundsType'] == 2:
                plt.scatter(X,Y,s= 7, c = C,cmap=matplotlib.cm.cool,alpha=ops, marker = "s")#,vmin=boundsDict['bounds'][0],vmax=boundsDict['bounds'][1] )
            else:
                plt.scatter(X,Y,s= 25, c = C,cmap=matplotlib.cm.cool,alpha=1,marker = "|")#,vmin=boundsDict['bounds'][0],vmax=boundsDict['bounds'][1])
         
        # Use clim to limit colors displayed
        plt.clim(boundsDict['bounds'][0],boundsDict['bounds'][1])  
